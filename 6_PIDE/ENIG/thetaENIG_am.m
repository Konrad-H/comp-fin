function Price =  ThetaEu_EVG(S0,T,r,K,  pEVG,theta, flag, M, N,D,U,epsilon )
% Price a European Call (or Put) Option using FD SCHEME - THETA
% Model: ENIG
% Framework: General Levy - LogPrice

%% For ENIG Model
sigmaVG=pEVG(1); thetaVG=pEVG(2);  kVG=pEVG(3); sigmaGBM=pEVG(4);
Anu=thetaVG/(sigmaVG^2);
Bnu=sqrt(thetaVG^2+sigmaVG^2/kVG)/(sigmaVG^2);
Cnu = sqrt(thetaVG^2+sigmaVG^2/kVG)/(pi*sigmaVG*sqrt(kVG));
nu=@(y) Cnu./(abs(y)).*exp(Anu*y).*besselk(1,Bnu*abs(y));
