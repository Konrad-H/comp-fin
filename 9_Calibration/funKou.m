function [Error,Price]=funKou(x,spot,strike,rf,maturity,pmkt)
...
% parameters
sigma=x(1); p=x(2); lambdaK=x(3); lambdap=x(4); lambdam=x(5);

V=@(v)-sigma^2/2*v.^2+1i*v*lambdaK.*...
    (p./(lambdap-1i*v)-(1-p)./(lambdam+1i*v)); % without drift
