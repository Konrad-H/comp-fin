function [Error,Price]=funENIG(x,spot,strike,rf,maturity,pmkt)
...
sigmaIG=x(1); thetaIG=x(2); kIG=x(3); sigmaGBM=x(4);
% risk-neutral characteristic exponent
V=@(v) -(sigmaGBM^2/2)*v.^2 ... %Extended GBM part
 +1/kIG-sqrt(1+v.^2*sigmaIG^2*kIG-2i*thetaIG*v*kIG)/kIG; % without drift
